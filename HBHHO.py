...
class HBHHO:
    ...
    def calculateSmellIntensity(self, hawks, index, rabbit_location):
        epsilon = np.finfo(np.float32).eps

        S = (hawks[index] - hawks[(index+1) % len(hawks)] + epsilon)**2
        d = hawks[index] - rabbit_location + epsilon
        intensity = np.random.random() * (S / (4*np.pi * d * d))
    ...
    def optimize(self):
        ...
        for iter in range(self.nIters):
            ...
            alpha = self.C * np.exp((-iter/self.nIters))  # Honey Badger alpha
            ...
            for index in range(self.noSearchAgents):
                ...
                if np.abs(escape_energy) >= 1:
                    ...
                    fitness = self.fitnessFunction(hawks[index, :])
                    # Calculate smell Intensity
                    intensity = self.calculateSmellIntensity(
                        hawks, index, rabbit_location)

                    # Calculate di
                    di = rabbit_location - hawks[index, :]
                    ...
                    new_position = rabbit_location + F * self.beta * intensity * rabbit_location + F * np.random.random() * \
                        alpha * di * np.abs(np.cos(4 * np.pi * np.random.random())
                                            * (1 - np.cos(4*np.pi*np.random.random())))

                    new_fitness = self.fitnessFunction(new_position)

                    if new_fitness <= fitness:
                        hawks[index, :] = new_position.copy()

                    # q = np.random.random()
                    # random_Hawk_index = np.floor(self.noSearchAgents * np.random.random())
                    # random_hawk = hawks[random_Hawk_index.astype('int'),:]

                    # if q < 0.5 :
                    #   hawks[index,:] = random_hawk - np.random.random() * np.abs(random_hawk - 2* np.random.random() * hawks[index,:] )

                    # elif q >=0.5:
                    #   hawks[index,:] = (rabbit_location - hawks.mean(0)) - np.random.random() * ((self.ub-self.lb)  * np.random.random() + self.lb )
                    ...
                    # Soft Besiege Dive
                        ...
                        new_position = rabbit_location - escape_energy * \
                            np.abs(jump_strength *
                                   rabbit_location - hawks.mean(0))

                        if self.fitnessFunction(new_position) < fitness:
                            hawks[index, :] = new_position
                        else:
                            levyFlight_position = rabbit_location - escape_energy * \
                                np.abs(jump_strength * rabbit_location - hawks.mean(0)) + np.multiply(
                                    np.random.randn(self.dim), self.LevyFlight(self.dim))
